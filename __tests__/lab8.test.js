describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    sRoot = await prodItem.getProperty('shadowRoot');
    const button = await sRoot.$('button');
    await button.click();
    const inText = await button.getProperty('innerText');
    let tValue = await inText.jsonValue();
    expect(tValue).toBe('Remove from Cart');
  }, 2500);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let button = await sRoot.$('button');
      let bInText = await button.getProperty('innerText');
      let bValue = await bInText.jsonValue();
      expect(bValue).toBe('Remove from Cart');
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let bInText = await button.getProperty('innerText');
      let bValue = await bInText.jsonValue();
      expect(bValue).toBe('Add to Cart');
    }
